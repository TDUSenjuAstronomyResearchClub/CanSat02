...
while True:
    ...
    if result==b'landing' or processing_time >= 1200: #地上局からコマンドを受け取った or 20分以上たったら自動的に着地判定にする
        ser.write(b'landing now')
        ser.close()
        log('Cansat landing now',None,filename)
        break
...
#---溶断回路を動かす（キャリア開放）---
ser = serial.Serial(PORT, BAUD_RATE,timeout=0.1)
ser.write(b'fusing circuit start(carrier)')  #溶断回路を動かすと地上局に伝える
ser.close()

log('using circuit start(carrier)',None,filename)

GPIO.output(carrier, 1)    # 溶断開始
time.sleep(10)               # 10秒待機
GPIO.output(carrier, 0) #溶断停止

log('using circuit end(carrier)',None,filename)

ser = serial.Serial(PORT, BAUD_RATE,timeout=0.1)
ser.write(b'fusing circuit end(carrier)')  #溶断回路を動かすと地上局に伝える
ser.close()
...
current_value = SoilMoisture.SoilMois_result()
if current_value == True: #距離センサがOSErrer出てないかの条件分岐
    ManualMain.manual_mode() #手動運転モードに切り替える
else:
    log('soil moisture',current_value,filename)
    ser = serial.Serial(PORT, BAUD_RATE,timeout=0.1)
    current_value_result = str(current_value)
    ser.write(current_value_result.encode('utf-8'))    #土壌水分センサの抵抗値を測る
